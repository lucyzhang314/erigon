...
func (g *Generator) GetReceipt(ctx context.Context, cfg *chain.Config, tx kv.Tx, block *types.Block, index int, optimize bool) (*types.Receipt, error) {
	...
	if optimize {
		...
		if err != nil {
			return nil, fmt.Errorf("ReceiptGen.GetReceipt: bn=%d, txnIdx=%d, %w, %T", block.NumberU64(), index, err, g.engine)
		}
		...
		getParentHash := core.GetHashFn(genEnv.header, genEnv.getHeader)
		for i, txn := range block.Transactions() {
			genEnv.ibs.SetTxContext(i)
			receipt, _, err = core.ApplyTransaction(cfg, getParentHash, g.engine, nil, genEnv.gp, genEnv.ibs, genEnv.noopWriter, genEnv.header, txn, genEnv.usedGas, genEnv.usedBlobGas, vm.Config{})
			if err != nil {
				return nil, fmt.Errorf("ReceiptGen.GetReceipt: bn=%d, txnIdx=%d, %w, %T, %x", block.NumberU64(), i, err, g.engine, txn.Hash())
			}
			receipt.BlockHash = block.Hash()
			if i == index {
				break
